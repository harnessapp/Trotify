...
let positionChartSelectedStart = "ALL";
    ...
    document.getElementById("raceDetailPanel").innerHTML = `
        <div class="race-detail" onclick="closeSizePopupAndRender('${escapeHtml(venue)}', '${escapeHtml(state)}', '${escapeHtml(dateValue)}', '${escapeHtml(raceNo)}')">
            <div class="race-detail-header">
                <div class="race-panel-eyebrow">Race detail</div>
                <div class="race-title-row">
                    <h2>${escapeHtml(venue)} R${escapeHtml(raceNo)}</h2>
                ...
                <h3>${escapeHtml(time)} • Race ${escapeHtml(raceNo)}${raceName ? ` • ${escapeHtml(raceName)}` : ""}</h3>
...
function renderPositionChart() {
    ...
    if (!chartRows.length) {
        ...
        chartRaceInstance.setOption({
            title: {
                text: "No categories have at least 100 starts",
                subtext:
...
function updatePositionChartHeading(rows) {
    ...
    if (description) {
        description.textContent =
            ...
            `100 starts after filtering.`;
    }

    if (note) {
        const totalStarts = rows.reduce(
            (sum, row) =>
                sum + (Number(row.starts) || 0),
            0
        );
...
function setPositionChartState(value) {
    ...
    positionChartSelectedStart = "ALL";
...
function startSmoothRaceChartSegment() {
    ...
    const fromFrame = frames[chartRaceFrameIndex];
    const toFrame = frames[chartRaceFrameIndex + 1];

    const segmentStart = performance.now();
    const segmentDuration = chartRaceSpeedMs;
    ...
    chartRaceInstance.setOption(
        {
            ...
            yAxis: {
                ...
                axisLabel: {
                    ...
                    fontWeight: 850,
                    width: RACE_CHART_SOCIAL_MODE ? 134 : 170,
                    overflow: "truncate"
                },
            ...
            series: [
                {
                    ...
                    data: top.map(item => ({
                        ...
                        itemStyle: {
                            color: getRaceChartColour(item.name),
                    ...
                    label: {
                        ...
                        position: "right",
                        color: "#f4f7fb",
                        fontSize: RACE_CHART_SOCIAL_MODE ? 20 : 15,
                        fontWeight: 900,
                        valueAnimation: true,

                        formatter: params =>
                            formatRaceChartValue(params.value)
                    },
...
function renderInterpolatedRaceChartFrame(
    ...
    const interpolated = [...allNames].map(name => {
        const startValue =
            fromMap.get(name) || 0;
   ...
   true
    ...
    if (periodEl) {
        ...
        periodEl.textContent = RACE_CHART_SOCIAL_MODE
...
function toggleRaceChartPlay() {
    ...
    if (chartRaceFrameIndex >= frames.length - 1) {
        chartRaceFrameIndex = 0;
        renderRaceChartFrame(0);
    }
...
function setRaceChartFrame(value) {
    stopRaceChartAnimation();
    renderRaceChartFrame(Number(value));
}

function setRaceChartState(state) {
    chartRaceSelectedState = state || "ALL";
...
function getRaceChartTitle() {
    ...
    return `${entity} ${getRaceChartMetricLabel()} Race`;
}
...
function setRaceChartType(type) {
    ...
    cards[
        typeOrder[chartRaceSelectedType] ?? 0
